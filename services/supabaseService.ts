...
export const SupabaseService = {
    ...
    try {
      ...
      rawSettings.forEach(s => {
        if (s.key === 'AdminPassword') {
          ...
          const [color, dept, lineId] = desc.includes('|') ? desc.split('|') : [desc, '', ''];
  ...
  // 2.1 刪除多個專案 (包含刪除關聯的 Logs)
  deleteProjects: async (projectIds: string[]): Promise<void> => {
    try {
      // Step 1: 先刪除關聯的工時紀錄 (Logs)
      const { error: logError } = await supabase
        .from(CONFIG.SUPABASE.TABLES.LOGS)
        .delete()
        .in('projectid', projectIds);

      if (logError) throw new Error(`Delete Related Logs Error: ${logError.message}`);
  ...
  upsertGlobalEngineer: async (eng: GlobalEngineer): Promise<void> => {
    try {
      const payload = {
        key: `User:${eng.name}`,
        value: eng.password, 
        description: `${eng.color}|${eng.department || ''}|${eng.lineId || ''}`
      };
      const { error } = await supabase
