        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[300] flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 max-h-[80vh] flex flex-col">
            ...
            <div className="bg-slate-50 p-4 rounded-lg mb-6 border border-slate-200">
              ...
              <div className="mb-6">
                <label className="text-xs font-bold text-slate-500 uppercase block mb-1">LINE 使用者 ID</label>
                <div className="flex gap-2">
                  <input value={editingEngineer.lineId || ''} onChange={e => setEditingEngineer({ ...editingEngineer, lineId: e.target.value })} className="flex-1 border border-brand-200 rounded px-3 py-2 text-sm font-mono focus:border-brand-500 outline-none" placeholder="填入經理或生管的 LINE ID" />
                  <a href="https://manager.line.biz/" target="_blank" rel="noreferrer" title="如何取得 LINE ID?" className="bg-slate-200 text-slate-600 px-3 py-2 rounded text-xs flex items-center hover:bg-slate-300">
                    <i className="fa-solid fa-circle-question"></i>
                  </a>
                </div>
                <p className="text-[10px] text-slate-400 mt-1">※ 僅管理層需要填寫，用於接收 AI 週報與執行查詢</p>
              </div>
              ...
              {globalEngineers.length === 0 ? (
                ...
                <table className="w-full text-sm text-left table-fixed">
                  <thead className="text-xs text-slate-500 uppercase sticky top-0 z-50 bg-white shadow-sm ring-1 ring-slate-900/5">
                      <th className="py-2 w-[20%] pl-2 bg-white border-b border-slate-100">姓名</th>
                      <th className="py-2 w-[15%] bg-white border-b border-slate-100">部門</th>
                      <th className="py-2 w-[30%] bg-white border-b border-slate-100">LINE ID</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {globalEngineers.map(eng => (
                      <tr key={eng.name} className="hover:bg-slate-50">
                        <td className="py-2 font-bold text-slate-700 w-[20%] pl-2 truncate" title={eng.name}>{eng.name}</td>
                        <td className="py-2 text-slate-500 w-[15%] truncate">{eng.department || '未設定'}</td>
                        <td className="py-2 font-mono text-slate-400 text-[10px] w-[30%] truncate">{eng.lineId || <span className="text-slate-300 italic">未填</span>}</td>
